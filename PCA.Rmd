...
```{r setupbatch, echo=FALSE, eval=FALSE}
##you need to run this once you download the GitHub repo to set up the intermediate files
##used below

source("OSCAmultisample.R")
...
- it will be good to familiarize yourself with the OSCA Multisample Book
...
```{r batcheffects1, echo=FALSE}
##if these are missing source the file OSCAmultisample.R
load("pcc.rda")
load("batch.rda")
load("pcaComb.rda")
...
```{r nocorrect}
 multR2raw = sapply(1:9, function(x) {
     summary(lm(pcaComb$x[,x]~ batch - 1))$adj.r.squared
 })
...
## After correction 
 - we now carry out the same calculation - regress each of the first 9 PCs against `batch`
 and report the multiple R^2 
...
##some code to plot, should you want to 
plot(pcc$x[,1], pcc$x[,2], col=ifelse(combined$batch=='A', "goldenrod","slateblue"))
